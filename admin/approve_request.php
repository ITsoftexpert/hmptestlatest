...
<?php
    ...
    if (isset($_GET['approve_request'])) {
        ...
        if ($update_request) {
            ...
            $data = [];
            $data['template'] = "approve_buyer_request";
            $data['to'] = $seller_email;
            $data['subject'] = "$site_name: Your request has been successfully approved.";
            $data['user_name'] = $seller_user_name;
            $data['request_url'] = "requests/view_offers?request_id=".$request_id;
            send_mail($data);
