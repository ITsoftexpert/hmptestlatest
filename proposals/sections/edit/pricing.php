...
<div class="form-group row proposal-price justify-content-center">
  <div class="col-md-7">
    ...
    <div class="input-group">
      ...
      <input type="number" class="form-control" form="pricing-form" name="proposal_price" min="<?= $min_proposal_price; ?>" value="<?= $d_proposal_price; ?>" required="">
    </div>
...
<script>
  $(document).ready(function() {
    ...
    if (enable_delivery == 0) {
      ...
      <?php } elseif ($d_proposal_price != "0" and !isset($_POST["fixedPriceOff"])) { ?>
      ...
      $('.packages input[name="proposal_packages[1][description]"]').removeAttr('required');
      $('.packages input[name="proposal_packages[2][description]"]').removeAttr('required');
      $('.packages input[name="proposal_packages[3][description]"]').removeAttr('required');
    ...
    // working code when i going to add new attribute start
    $(".insert-attribute").on('click', function(event) {
      ...
      // working code when i going to add new attribute end



      function processAttributeRequest(attribute_name, status = true) {
        // working 
        $.ajax({
          method: "POST",
